...
func NewEchoDatabase(dbPath string) (*EchoDatabase, error) {
	...
	return &EchoDatabase{db: db, operations: make([]Operation, 0)}, nil
}

func (d *EchoDatabase) CreateUser(name, email string, age int) (map[string]interface{}, error) {
	start := time.Now()
...
func (d *EchoDatabase) GetUser(id int64) (map[string]interface{}, error) {
	start := time.Now()
...
func (d *EchoDatabase) GetAllUsers() ([]map[string]interface{}, error) {
	start := time.Now()
...
func (d *EchoDatabase) UpdateUser(id int64, name, email string, age int) (bool, error) {
	start := time.Now()
...
func (d *EchoDatabase) DeleteUser(id int64) (bool, error) {
	start := time.Now()
...
func (d *EchoDatabase) Benchmark(count int) (map[string]interface{}, error) {
	start := time.Now()
	tx, err := d.db.Begin()
	if err != nil {
		return nil, err
...
func (d *EchoDatabase) recordOperation(opType string, duration float64) {
	d.operations = append(d.operations, Operation{Type: opType, Duration: duration, Timestamp: time.Now()})
}

func (d *EchoDatabase) GetPerformanceReport() map[string]map[string]interface{} {
...
func (d *EchoDatabase) Close() error {
	return d.db.Close()
}
